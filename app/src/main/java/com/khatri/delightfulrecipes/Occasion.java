...
/**
 * Created by Faiyyaz on 29-Sep-16.
 */
public class Occasion extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            cursor = db.rawQuery("SELECT * FROM occasion ORDER BY name ASC", null);
    ...
    public void search_db(View v) {
        ...
        if (!edit_db.equals("")) {
            try {
                cursor = db.rawQuery("SELECT * FROM occasion WHERE name LIKE ?",
                        new String[] { "%" + edit_db + "%" });
            ...
            try {
                cursor = db.rawQuery("SELECT * FROM occasion ORDER BY name ASC",
                        null);
                adapter = new SimpleCursorAdapter(
                        this,
                        R.layout.isi_lv,
